...
bribe_amount <- html_text(bribe_nodes) %>%
  ...
  stri_replace_all_fixed(",", "") %>% #replace all occurences
  as.numeric
...
```{r}
transaction <- html_nodes(bribe_html, ".transaction a") %>% #see class a on the inspect console
  html_text
...
scrape_bribe <- function(url){
  ...
  df <- data.frame(bribe_amount, transaction, dept, stringsAsFactors = FALSE)
...
```{r}
base_url <- "http://ipaidabribe.com/reports/paid?page=" #base url with the argument page= for choosing pages
pages <- seq(0, 40, by = 10) #can scrape certain pages 
```
...
```{r}
for (i in 2:length(pages)){   #start from second element because you have already created the first one
  # informative message about progress of loop
  message(i, '/', length(pages))
  # prepare URL
  url <- paste0(base_url, pages[i])
  # scrape website
  data_list[[i]] <- scrape_bribe(url) #save it as i element of a list in the the data frame, sebset list double braket, df single
  # wait a couple of seconds between URL calls
  Sys.sleep(2) #stop from over burdening the server, do nothing for two seconds
}
```
